function removeRowDivider() {
    if ($("#dashcontent").length) {
        $("#dashcontent > section").each(function() {
            // New structure: each section has a single .row
            var row = $(this).find("div.row").first();
            if (row.length && row.children().first().hasClass("span3")) {
                $(this).addClass("compact");
            }
            // Legacy support: consolidate multiple .row.divider groups if present
...
function ajaxSuccessCallback(event, xhr, settings) {
    ...
    if (settings.url.startsWith("json.htm?type=command&param=getdevices") || settings.url.startsWith("json.htm?type=command&param=getscenes")) {
        ...
        if (mainView && !mainView._machinonObserver) {
            ...
            mainView._machinonObserver.observe(mainView, { childList: true, subtree: true });
        }
