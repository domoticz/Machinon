...
| `warn_repeat` | Repeat device warnings | Controls how often the same device may warn you again: **Once per visit** shows the warning again after you reload the page; **Once a day** (the default) waits 24 hours before repeating it; **Only when it changes** stays quiet until the problem goes away and comes back. However you set this, a problem that clears and then comes back always warns again right away, and the warning icon next to the device name is shown regardless of this setting. The quiet period is remembered by the browser you are looking at Domoticz in, so a warning you have already seen on your computer can still show up once on your phone. | Toasts |
