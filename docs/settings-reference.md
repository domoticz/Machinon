...
| `rgbw_popup` | Machinon colour picker popup | Replaces Domoticz's own colour picker dialog with a Machinon-styled one for colour lights. On by default. Turning it off restores the original picker; the change takes effect after the page reloads. | Colour light devices |
