...
The library is built from [Icons8](https://icons8.com) artwork, drawn from two specific Icons8
styles: **Blue UI** (the source for the pack's Blue UI icons) and **Office M** (the source for
the pack's Color icons). The form asks for a link to the Icons8 icon you have in mind, and a
request that links an icon already available in one of those two styles can generally be added
as-is; a request for a shape that only exists in another Icons8 style takes more work, since it
first needs a matching Blue UI or Office M version to be found or drawn. The form also asks
whether the device has separate on and off states, whether it needs a small inset badge (like the
lightning bolt on an energy meter icon), and which color tint to use.
